...
def createZip(source, target, env):
    if os.getenv("OPENIRIS_CI_BUILD", False):
        ...
        # make_archive(zip_file_path.replace('.zip', ''), 'zip', archive_folder)
        final_relative_zip_basename_path = _out_zip_parent_path.joinpath(zip_file_path.stem).resolve()
        make_archive(final_relative_zip_basename_path, format='zip', root_dir=archive_folder)
